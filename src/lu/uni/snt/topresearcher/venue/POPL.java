...
public class POPL extends Conf
{
	...
	public POPL(String name, int lastNYears) {
		super(name, lastNYears);
	}
	
	public POPL(String name) {
		super(name);
	}

	public static void main(String[] args) 
	{	
		String venueName = "popl";
		
		Utils.toHtml_Fulllist(new POPL(venueName).toYearItems(), venueName);
		
		Utils.toHtml_Statistic(new POPL(venueName).rank(), new POPL(venueName, 10).rank(), new POPL(venueName, 5).rank(), venueName);
	}
